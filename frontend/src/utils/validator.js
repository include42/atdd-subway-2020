const validator = {
  path: {
    source: [(v) => !!v || '출발역 입력이 필요합니다.', (v) => v.id > 0 || '유효한 역이 아닙니다.'],
    target: [(v) => !!v || '출발역 입력이 필요합니다.', (v) => v.id > 0 || '유효한 역이 아닙니다.']
  },
  departureTime: {
    dayTime: [(v) => !!v || '시간 입력이 필요합니다.', (v) => (v === 'am' || v === 'pm') || '유효한 시간이 아닙니다.'],
    hour: [(v) => (v >= 1 && v <= 12) || '유효한 시간이 아닙니다.'],
    minute: [(v) => (v >= 0 && v <= 60) || '유효한 시간이 아닙니다.']
  },
